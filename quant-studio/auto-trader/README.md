...
已收进 `quant-studio/`，是 Quant Lab 的自动化运行器。把 `../../skills/company-deep-dive`、
`../../skills/quant-backtest`、`../../skills/trading-analyst` 三个 Skill 串成一个持续运转的循环，
...
| `strategy-kit/` | 内置策略片段库（翻译自 `../../../facts-hub` 的因子定义，全仓唯一源），`strategy_kit.py` 提供 list/show/render/compose——calibrating 阶段优先从这里选现成片段，而不是每次现场重新写策略。quant-studio 的 Rust 侧也读同一份 JSON |
...
```bash
# 首次使用：从模板建一份真实配置
cp quant-studio/auto-trader/config/strategies.example.json \
   quant-studio/auto-trader/config/strategies.json
# 编辑 strategies.json，填入 calibrating 阶段产出的 strategy_file / inputs

# 手动跑一轮（测试用）
python3 quant-studio/auto-trader/scripts/run_cycle.py

# 只跑某个标的
python3 quant-studio/auto-trader/scripts/run_cycle.py --symbol TSLA.US

# 生成本地监控 dashboard（深色主题，dashboard.html，不提交/不分享，浏览器直接打开看）
python3 quant-studio/auto-trader/scripts/render_dashboard.py
```
